...
IViewPort *GetViewPortInterface()
{
	...
	if ( !pViewPort )
	{
		pViewPort = **(IViewPort***) (SearchPattern( L"client.dll", "\x8B\x0D\x00\x00\x00\x00\x85\xC9\x74\x2D\x8B\x01" ) + 2);
...
void CClassMenu::OnCommand( const char *command )
{
	...
	{
		g_pEngineClient->ClientCmd( const_cast<char *>( command ) );
		...
		{
            g_pEngineClient->ClientCmd( "kill" );
		}
