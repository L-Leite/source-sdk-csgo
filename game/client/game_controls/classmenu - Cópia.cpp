...
{
	using fn_t = void(__thiscall*)( Frame*, Panel*, const char*, bool, bool );
	static fn_t pFn = nullptr;

	if ( !pFn )
	{	  
		pFn = (fn_t) SearchPattern( L"client.dll", "\x55\x8B\xEC\x51\x53\x56\x57\xFF\x75\x0C\x8B\xD9\xFF\x75\x08\xE8\x00\x00\x00\x00\xC7\x03\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8", "xxxxxxxxxxxxxxxx????xx????x????x????x????x????x????x????x????x" );
		ConsoleDebugW( L"vgui::Frame::Frame: %p\n", pFn );
	}

	pFn( this, parent, panelName, showTaskbarIcon, bPopup );
}

EditablePanel::EditablePanel( Panel *parent, const char *panelName )
{
	using fn_t = void( __thiscall* )( EditablePanel*, Panel*, const char* );
	static fn_t pFn = nullptr;

	if ( !pFn )
	{
		pFn = (fn_t) SearchPattern( L"client.dll", "\x55\x8B\xEC\x51\x53\x56\x57\xFF\x75\x0C\x8B\xD9\xFF\x75\x08\xE8\x00\x00\x00\x00\xC7\x03\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xA1\x00\x00\x00\x00", "xxxxxxxxxxxxxxxx????xx????x????x????x????x????x????x????x????" );
		ConsoleDebugW( L"vgui::EditablePanel::EditablePanel: %p\n", pFn );
	}

	pFn( this, parent, panelName );
}

IViewPort *GetViewPortInterface()
{
	static IViewPort* pViewPort = nullptr;

	if ( !pViewPort )
	{
		pViewPort = **(IViewPort***)( SearchPattern( L"client.dll", "\x8B\x0D\x00\x00\x00\x00\x85\xC9\x74\x2D\x8B\x01", "xx????xxxxxx" ) + 2 );
		ConsoleDebugW( L"pViewPort: %p\n", pViewPort );
	}
