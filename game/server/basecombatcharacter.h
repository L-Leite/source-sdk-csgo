...
#define WEAPON_MAX 60 // Uhh probably 
...
public:
	...
	int					Weapon_GetSlot( const char *pszWeapon, int iSubType = 0 ) const;  // Returns -1 if they don't have one
	virtual CBaseCombatWeapon *Weapon_GetSlot( int slot ) const;
	virtual CBaseCombatWeapon* Weapon_GetPosition( int position ) const;
	...
	virtual bool			HasEverBeenInjured( int a1 );
	virtual float			GetTimeSinceLastInjury( int a1 );
...
public:
	...
	virtual void		RemoveAllWearables();
	...
	virtual void		EquipWearable( CEconWearable* pWearable );
	virtual void		RemoveWearable( CEconWearable* pWearable );
	virtual int			FindWearableInSlot( loadout_positions_t position );
	virtual int			RemoveWearableInSlot( loadout_positions_t position );
...
protected:
	...
	uint8_t CombatCharPad00[ 0x3C ]; //Has a bunch of IntervalTimers isnide, idc about them

	float m_flTimeOfLastInjury;
	int m_nRelativeDirectionOfLastInjury;

	int CombatCharPad01[ 2 ];
