...
#include "inventoryupdatelistener.h"		
...
#define ARMS_MODEL_STRING_SIZE 128 

#define MATCH_STATS_MAX_ENTRIES 30
...
extern CCSPlayerStateInfo* g_pPlayerStateInfos;
...
//=============================================================================
class CCSPlayer : public CBaseMultiplayerPlayer, public ICSPlayerAnimStateHelpers, public IHasAttributes, public IInventoryUpdateListener
{
public:
	...
	virtual void		Spawn();
	virtual void		PostSpawnPointSelection();
	...
	virtual float		GetLayerSequenceCycleRate( CAnimationLayer* pAnimationLayer, int iSequence );

	virtual bool		FlexUnknown00( int a1, int a2 );
	...
	virtual bool		PlayerUnknown00( int a1 );

	virtual bool		StartObserverMode( int mode );
	virtual bool		SetObserverTarget( CBaseEntity * target );
	virtual void		ObserverUse( bool bIsPressed );
	...
	virtual void AwardAchievement( int iAchievement, int mapId );
...
public:
	virtual bool IsTaunting();
	virtual bool IsThirdPersonTaunt();
	virtual float GetTauntYaw();
	virtual void StopTaunting();

	virtual bool IsLookingAtWeapon();
	virtual bool IsHoldingLookAtWeapon();
	virtual bool StopLookingAtWeapon();
	...
	int GetAccount() { return m_iAccount; }
	...
	virtual void OnSwitchWeapons( CBaseCombatWeapon* pNewWeapon );

	virtual bool CanKickFromTeam( int iTeamNum );
	...
	virtual bool		ShouldPickupItemSilently();
	...
	virtual void SOCacheUnsubscribed( const CSteamID& steamId );
...
public:
	...
	void State_Set( CSPlayerState newState );
...
private:
	uint8 CSPlayerPad03[ 0x8 ];

	int	m_iDeathPose;
	int	m_iDeathFrame;

	uint8 CSPlayerPad04[ 0x5 ];

	bool m_bTeamChanged;		// Just allow one team change per round

	uint8 CSPlayerPad004[ 0x12 ];

	CNetworkArray( int, m_rank, 6 );

	uint8 CSPlayerPad05[ 0x2 ];

	uint16_t m_unMusicID;

	uint8 CSPlayerPad06[ 0xC ];
...
public:			  
	...
	CNetworkVar( bool, m_bIsGrabbingHostage );

	float m_fImmuneToGunGameDamageTime;
	bool m_bGunGameImmunity;
	bool m_bMadeFinalGunGameProgressiveKill;
	int m_iGunGameProgressiveWeaponIndex;
	int m_iNumGunGameTRKillPoints;
	int m_iNumGunGameKillsWithCurrentWeapon;
	int m_iNumRoundKills;

	uint8 CSPlayerPad08[ 0x8 ];

	float m_fMolotovUseTime;
	...
	float m_flGuardianTooFarDistFrac;

	int CSPlayerPad09;

	float m_flDetectedByEnemySensorTime;

	CNetworkVar( bool, m_bHasHelmet );				// Does the player have helmet armor
	CNetworkVar( bool, m_bHasHeavyArmor );
	...
	char m_szArmsModel[ ARMS_MODEL_STRING_SIZE ];
	...
	CNetworkArray( int, m_iMatchStats_KillReward, MATCH_STATS_MAX_ENTRIES );
	CNetworkArray( int, m_iMatchStats_LiveTime, MATCH_STATS_MAX_ENTRIES );
	CNetworkArray( int, m_iMatchStats_Deaths, MATCH_STATS_MAX_ENTRIES );
	CNetworkArray( int, m_iMatchStats_Assists, MATCH_STATS_MAX_ENTRIES );
	CNetworkArray( int, m_iMatchStats_HeadShotKills, MATCH_STATS_MAX_ENTRIES );
	CNetworkArray( int, m_iMatchStats_Objective, MATCH_STATS_MAX_ENTRIES );
	CNetworkArray( int, m_iMatchStats_CashEarned, MATCH_STATS_MAX_ENTRIES );
	...
	int m_iShotsFiredInRound; // Didn't test that much, but it should be in the round
	...
	bool m_bIsRespawningForDMBonus;

	uint8 CSPlayerPad19[ 0xA ];
...
private:
	...
	bool			m_bIsInRebuy;	  
	RebuyStruct		m_nextRebuyStruct; // This contains the shit we're buying in this current round
	RebuyStruct		m_rebuyStruct; // "rebuy" command will use this structure, and has the stuff you bought last round
...
private:
	...
	uint8 CSPlayerPad23[ 0x8 ];

	//A list of damage given
	CUtlLinkedList< CDamageRecord *, int >	m_DamageGivenList; // They seem to be having both given and taken damage in here,
															   // but not like this?
														
	int CSPlayerPad24; // A 'fuck you' pad from Valve
...
public:			
	...
	uint8 CSPlayerPad25[ 0x80 ];

	bool m_bIsRescuing;

	uint8 CSPlayerPad26[ 0x68 ];

	bool m_bPlayerDominated[ 65 ];
	bool m_bPlayerDominatingMe[ 65 ];
...
inline void CCSPlayer::State_Set( CSPlayerState newState )
{	
...
inline void CCSPlayer::SetClass( int iClass )
{
	m_iClass = iClass;
}
