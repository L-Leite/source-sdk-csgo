...
// Hacky
#if defined ( USES_ECON_ITEMS )	|| defined ( CSTRIKE_DLL ) && !defined ( CLIENT_DLL ) // TEMP UNTIL WE REVERSE CLIENT
...
#else 
#define BASECOMBATWEAPON_DERIVED_FROM		CBaseFlex
#endif 
...
public:
	...
	virtual CBaseCombatWeapon* HolsterOnDetach(); // Not sure if it returns a CBaseCombatWeapon
	...
	virtual float			GetAccuracyFishtail();
...
public:
	...
	virtual const char*		GetWorldDroppedModel() const;
	...
	virtual int				GetReserveAmmo( int ammoType ) const; // Either use 1 to get primary ammo or 2 to get secondary (untested)
	...
	// Idk if this is only on server dll
	void					GiveAmmo( int a1, int iCount, bool bSuppressSound, int a2 );
...
// Client only methods
	...
	virtual bool			IsSilentPickupThirdperson( CBaseCombatCharacter* pOwner );
...
public:
	...
	CNetworkVar( EHANDLE, m_hWeaponWorldModel );
...
public:
	...
	CNetworkVar( int, m_iPrimaryReserveAmmoCount );
	CNetworkVar( int, m_iSecondaryReserveAmmoCount );
...
public:
	...
	CNetworkVar( bool, m_iWeaponOrigin );
