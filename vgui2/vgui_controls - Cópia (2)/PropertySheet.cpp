...
//-----------------------------------------------------------------------------
void PropertySheet::AddPage(Panel *page, const char *title, char const *imageName /*= NULL*/, bool showContextMenu /*= false*/, int nInsertBefore /*= -1*/ )
{
	if (!page)
		return;
	...
	PageTab *tab = new PageTab(this, "tab", title, imageName, _tabWidth, page, m_bContextButton && showContextMenu, hoverActivatePageTime );
	if ( m_bDraggableTabs )
	{
	...
	Page_t info;
	info.page = page;
	info.contextMenu = m_bContextButton && showContextMenu;
