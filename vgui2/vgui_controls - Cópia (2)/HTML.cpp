...
class HTMLPopup : public vgui::Frame
{
	...
	public:
		PopupHTML( Frame *parent, const char *pchName, bool allowJavaScript , bool bPopupWindow, uint unk ) : HTML( parent, pchName, allowJavaScript, bPopupWindow, unk ) { m_pParent = parent; }
...
public:
	...
	{
		m_pHTML = new PopupHTML( this, "htmlpopupchild", true, true, 0 );
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
HTML::HTML(Panel *parent, const char *name, bool allowJavaScript, bool bPopupWindow, uint unk) : Panel(parent, name), 
...
m_HideTooltip( this, &HTML::BrowserHideToolTip )
{
	...
	if ( m_SteamAPIContext.SteamHTMLSurface() )
	{
		...
		SteamAPICall_t hSteamAPICall = m_SteamAPIContext.SteamHTMLSurface()->CreateBrowser( "ass"/*surface()->GetWebkitHTMLUserAgentString()*/, NULL );
...
void HTML::BrowserNeedsPaint( HTML_NeedsPaint_t *pCallback )
{
	...
	// Fuck you too, wont bother making you compilable
