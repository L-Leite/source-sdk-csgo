...
DmxElementUnpackStructure_t* vgui::Panel::s_pUnpackParams = nullptr;
...
wchar_t const *Panel::KeyCodeModifiersToDisplayString( KeyCode code, int modifiers )
{
	...
	_swprintf( unicode, L"%S%s", sz, Panel::KeyCodeToDisplayString( (KeyCode) code ) );
	return unicode;
}
...
void Panel::OnDraggablePanelPaint()
{
	...
	if ( m_pDragDrop->m_DragPanels.Count() > 1 )
	{
		...
		_swprintf( sz, L"[ %i ]", m_pDragDrop->m_DragPanels.Count() );
